...
| B-027 | 💡 | Remove or document unused nginx `/openapi/` and `/scalar/` blocks if docs move under `/api` | T4.5 review Nice: browser traffic now uses `/api/` block; dedicated doc proxy blocks may be redundant. | T4.1 OpenAPI/Scalar polish. |
